...
class SuperCoderCLI(object):
    ...
    def returnLangNames(self):
        ...
        self.languages = [l for l in self.soup.find(id="repo-tabs").stripped_strings]
        
    def returnData(self, lang):
        """Search and stores the data of the projects selected language"""

        for r in range(self.soup.find(class_="tab-content").contents.count("\n")):
            self.soup.find(class_="tab-content").contents.remove("\n")

        self.names = [n.string for n in
                            self.soup.find(class_="tab-content").contents[lang].find_all("a")]
        
        self.links = ["http://github.com"+n.get('href') for n in
                            self.soup.find(class_="tab-content").contents[lang].find_all("a")]
                
        self.issues = [i.string for i in self.soup.find(class_="tab-content").contents[lang].find_all("span")]

        self.descriptions = ["No description available." if str(i)=="<p></p>"
                                            else i.string for i in self.soup.find(class_="tab-content").contents[lang].find_all("p")]
    ...
    def showData(self):
        ...
        print(self.issues[self.count]+" Issues\n")
